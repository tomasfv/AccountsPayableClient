...
const AppLayout: React.FC = () => {
  ...
  const roleColors: Record<string, string> = {
    ...
    Approver: 'bg-purple-600/20 text-purple-400',
    Submitter: 'bg-slate-600/20 text-slate-400',
  }
      ...
      <aside className="w-60 flex-shrink-0 flex flex-col bg-surface-card border-r border-surface-border">
        ...
        <div className="px-5 py-5 border-b border-surface-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-white leading-none">AP Manager</p>
              <p className="text-xs text-slate-500 mt-0.5">Accounts Payable</p>
            </div>
          </div>
        </div>
          ...
          <div className="px-3 py-4 border-t border-surface-border">
            <div className="flex items-center gap-3 px-3 py-2 rounded-lg">
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className={`badge text-[10px] mt-0.5 ${roleColors[user.role] ?? ''}`}>
                  {user.role}
                </span>
              </div>
            </div>
            <button
              ...
              className="btn-secondary w-full justify-center mt-2 !text-slate-500 hover:!text-slate-300"
            >
              Sign out
            </button>
          </div>
